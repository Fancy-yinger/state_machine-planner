...
void AStar::initGridMap(shared_ptr<SDFMap> occ_map, const Eigen::Vector3i pool_size)
{
...
double AStar::getDiagHeu(GridNodePtr node1, GridNodePtr node2)
{
    ...
    // 【新增关键代码】 增加 Z 轴惩罚权重
    // 让算法认为：垂直方向每走一格，相当于水平方向走 5 格
    // 这会强迫它极力避免 Z 轴变化，除非水平方向完全堵死
    double z_penalty_weight = 2.0; 
    h += abs(node1->index(2) - node2->index(2)) * z_penalty_weight;
...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt, Vector3d end_pt, Vector3i &start_idx, Vector3i &end_idx)
{
    ...
    if (!Coord2Index(start_pt, start_idx) || !Coord2Index(end_pt, end_idx)) {
        printf("Error: Start or end point outside map boundaries!\n");
    ...
    // 检查索引是否在有效范围内
    auto isValidIndex = [&](const Vector3i& idx) {
        return idx(0) >= 0 && idx(0) < POOL_SIZE_(0) &&
               idx(1) >= 0 && idx(1) < POOL_SIZE_(1) && 
               idx(2) >= 0 && idx(2) < POOL_SIZE_(2);
    };
    
    if (!isValidIndex(start_idx) || !isValidIndex(end_idx)) {
        printf("Error: Converted indices out of pool range!\n");
        printf("Start_idx: (%d, %d, %d), End_idx: (%d, %d, %d)\n",
...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt)
{
    ...
    // 对于长距离搜索，增加步长
    double adaptive_step_size = std::max(step_size, (end_pt - start_pt).norm() / 50.0);
    adaptive_step_size = std::min(adaptive_step_size, 50.0); // 最大50m步长

    step_size_ = adaptive_step_size;
        ...
        for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
                for (int dz = -1; dz <= 1; dz++)
                {
                    ...
                    // 【新增关键代码】 如果 Z 轴发生变化 (dz != 0)，大幅增加代价
                    if (dz != 0) {
                        // 惩罚系数，建议 2.0 到 5.0 之间
                        // 设置得越高，它越不愿意爬升
                        static_cost *= 2.0; 
                    }
