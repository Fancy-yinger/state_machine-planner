/*
 * @Function:Obj Planner:Bspline Optimizer
 * @Author:Fancy
 * @Date:2026 01
 */
...
namespace obj_planner
{
  ...
  void BsplineOptimizer::setParam()
  {
    ...
    // ========== 【关键参数配置】消除波浪形震荡 ==========
    // 核心思路：大幅提高平滑权重，让优化器优先生成平滑轨迹
    lambda1_ = 40.0;  // 【核心】平滑权重 (必须大！抑制波浪)
    lambda2_ = 40.0;  // 避障权重 (与平滑权重相当，不要大太多)
    lambda3_ = 0.001;
    lambda4_ = 0.1;
    dist0_   = 0.5;
    max_vel_ = 1.5;   // 限制最大速度，避免生成过激轨迹
    max_acc_ = 1.0;   // 降低最大加速度，与实际安全参数一致
    order_   = 3;     // 必须是3 (EgoPlanner上层逻辑硬编码了3阶)
    // ========== 参数配置结束 ==========
    ...
    do
    {
     ...
     // double total_time_ms = (t2 - t0).toSec() * 1000;
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        {
          Eigen::Vector3d check_pt = traj.evaluateDeBoorT(t);
          flag_occ = grid_map_->getInflateOccupancy(check_pt);
          if (flag_occ)
          {
            // ========== 【新增调试打印】定位"隐形障碍物" ==========
            Eigen::Vector3d collide_pt = traj.evaluateDeBoorT(t);
            Eigen::Vector3d start_pt = cps_.points.col(order_);
            double dist_to_start = (collide_pt - start_pt).norm();

            std::cout << "\033[31m[DEBUG] Collision detected:\033[0m"
                      << " t=" << t
                      << " Pos=(" << collide_pt.x() << ", " << collide_pt.y() << ", " << collide_pt.z() << ")"
                      << " dist_to_start=" << dist_to_start
                      << " is_at_start=" << (dist_to_start < 0.5 ? "YES【起点碰撞】" : "NO")
                      << std::endl;
            // ===================================
