...
{
  auto t1 = std::chrono::high_resolution_clock::now(); // 【新增】开始计时
  ...
  for (int x = esdf_min_(0); x <= esdf_max_(0); ++x)
    for (int y = esdf_min_(1); y <= esdf_max_(1); ++y)
      ...
      auto t2 = std::chrono::high_resolution_clock::now(); // 【新增】结束计时
      auto duration = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
      
      // 【新增】打印耗时
      // 如果超过 10ms (10000us)，说明地图更新严重阻塞了线程
      if (duration > 10000) { 
          std::cout << "⚠️ ESDF Update took: " << duration / 1000.0 << " ms. Range: " 
                    << (esdf_max_ - esdf_min_).transpose() << std::endl;
      }
...
void SDFMap::initMap(double x_size,double y_size,double z_size,double resolution_, Eigen::Vector3d org,double inflate_values)
{
  ...
  /* ---------- get parameter ---------- */
    ...
    // ========== 详细内存使用计算 ==========
    size_t total_voxels = grid_size(0) * grid_size(1) * grid_size(2);
    size_t memory_required = total_voxels * sizeof(double) * 8; // 8个缓冲区
    
    std::cout << "=== ESDF Map Memory Usage ===" << std::endl;
    std::cout << "Grid size: " << grid_size.transpose() << std::endl;
    std::cout << "Total voxels: " << total_voxels << std::endl;
    std::cout << "Estimated memory: " << memory_required / (1024*1024) << " MB" << std::endl;
    
    if (memory_required > 500 * 1024 * 1024) {
        std::cout << "WARNING: Large memory usage detected!" << std::endl;
        std::cout << "Consider reducing map size or increasing resolution." << std::endl;
    } else if (memory_required > 100 * 1024 * 1024) {
        std::cout << "Moderate memory usage - acceptable for real-time planning" << std::endl;
    } else {
        std::cout << "Light memory usage - good for real-time planning" << std::endl;
    }
    // ========== 内存监控完成 ==========
  ...
  std::cout << "ESDF map initialization completed!" << std::endl;
...
void SDFMap::raycastProcess()
{
  ...
  // 【优化】注释掉频繁打印的ESDF边界信息，避免刷屏
  // std::cout << "cache_voxel_ size =" << cache_voxel_.size() << std::endl;
  // ROS_DEBUG_THROTTLE(5.0, "ESDF bounds: min=[%d,%d,%d], max=[%d,%d,%d]",
  //                    esdf_min_(0), esdf_min_(1), esdf_min_(2),
  //                    esdf_max_(0), esdf_max_(1), esdf_max_(2));
